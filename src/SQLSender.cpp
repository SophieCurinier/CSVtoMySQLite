...
class SQLSender
{
    private:
        ...
        void insertRequest(sqlite3* dataBase, string table, Event* event){
            openingDataBase = sqlite3_open(filename.c_str(), &dataBase);
            if (openingDataBase != SQLITE_OK){
                LogFile::instance()->logError(Time::getCurrentTime() + " - Erreur lors de l'ouverture de la base de donnée : " +  sqlite3_errmsg(dataBase));
                closingLinkDataBase(dataBase);
            }
            sqlite3_stmt* stmt ;
            char *zErrMsg = 0;
            ...
            if (preparingStatement != SQLITE_OK){
                LogFile::instance()->logError(Time::getCurrentTime() + " - Erreur lors de la selection de la requête suivante : " +  sqlite3_errmsg(dataBase));
                closingLinkDataBase(dataBase);
                exit (EXIT_FAILURE);
            }
            ...
            if (!isAlreadyInDataBase){
                ...
                if (preparingStatement != SQLITE_OK){
                    LogFile::instance()->logError(Time::getCurrentTime() + " - Erreur lors de la requête d'insertion : " +  zErrMsg + "\n");
